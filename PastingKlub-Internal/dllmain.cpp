...
void DrawMenu() {
	...
	if (ZeroGUI::Window("PastingKlub", &pos, Vector2{ 160.0f, 300.0f }, ShowMenu))
	{
		//Simple Tabs
		static int tab = 0;
		/*if (ZeroGUI::ButtonTab("Aimbot", Vector2{ 110, 25 }, tab == 0)) tab = 0;*/
		if (ZeroGUI::ButtonTab("Visuals", Vector2{ 110, 25 }, tab == 1)) tab = 1;
		...
		if (tab == 0) {
			//Some Elements
			/*ZeroGUI::NextColumn(130.0f);*/
			/*ZeroGUI::Checkbox("Aimbot", &Settings::MemoryAim);*/
			/*ZeroGUI::Checkbox("Silent Aim", &Settings::SilentAim);*/
			/*ZeroGUI::SliderFloat("Fov Size", &FovSize, .0f, 0.f);*/
		...
		if (tab == 1) {
			...
			ZeroGUI::Checkbox("Box", &Settings::BoxESP);
			ZeroGUI::Checkbox("Skeleton", &Settings::SkeletonESP);
			ZeroGUI::Checkbox("Line", &Settings::LinesESP);
			/*ZeroGUI::Checkbox("Name", &Settings::PlayerNameESP);*/
			ZeroGUI::Checkbox("Distance", &Settings::DistanceESP); 
			...
			/*ZeroGUI::SliderInt((char*)"ESP Go UP", &Settings::upwards, -1000, 1000);*/
			/*ZeroGUI::Checkbox("AimWhileJumping", &Settings::AimWhileJumping);*/
			//ZeroGUI::Checkbox("Level Esp", &Settings::LevelESP);
			//ZeroGUI::Checkbox("Kills Esp", &Settings::KillsESP);
		/*	ZeroGUI::Checkbox("Platform", &Settings::PlatformESP);*/
			/*ZeroGUI::Checkbox("Weapon", &Settings::EnemyWeaponESP);*/
	...
	for (int a = 0; a < levels.Num(); a++) {
		...
		for (int i = 0; i < actors.Num(); i++) {
			...
			if (actor->IsA(ue::cached::objects::actor::FortPlayerPawnAthena)) {
				...
				ue::FLinearColor col = ue::FLinearColor(255.f, 0.f, 0.f, 0.f);
				...
				if (!IsNotInScreen(1920, 1080, head2_w2s)) {
					if (Settings::PlayerNameESP) {
						...
						if (name.c_str()) {
							...
							calculation = Vector2(bottom_w2s.x, bottom_w2s.y - Settings::upwards);
					...
					if (Settings::DistanceESP) {
						...
						TextOffset_Y += static_canvas->K2_TextSize(ue::FString(wdistanceesp)).y;
						calculation = Vector2(bottom_w2s.x, bottom_w2s.y + 15);
					...
					if (Settings::EnemyWeaponESP) {
						if (actor->CurrentWeapon()->WeaponData()) {
							...
							if (weaponname) {
								...
								calculation = Vector2(head_w2s.x, head_w2s.y - 10);
				...
				if (Settings::SkeletonESP) {
					...
					Vector2 LeftShoulder = PlayerController->WorldToScreen(actor->GetBone(38));
					...
					Vector2 LeftHand = PlayerController->WorldToScreen(actor->GetBone(62));
					Vector2 RightHand = PlayerController->WorldToScreen(actor->GetBone(33));
					...
					Vector2 LeftFoot = PlayerController->WorldToScreen(actor->GetBone(81));
...
VOID Main()
{
	...
	ue::cached::signatures::GWorld = ue::cached::Base + 0xef005e0; //sdkUssage->PatternScan(ue::cached::Base, xorstr("48 89 05 ? ? ? ? 48 8B 4B 78"), 7, true); //48 89 05 ? ? ? ? 48 8B 4B 78
	if (!ue::cached::signatures::GWorld) {
		MessageBoxA(HWND_DESKTOP, xorstr("GWorld"), xorstr("Fortnite  "), MB_ICONERROR);
	}
	ue::cached::signatures::GetBoneMatrix = sdkUssage->PatternScan(ue::cached::Base, xorstr("E8 ?? ?? ?? ?? F2 0F 10 57 48 48 8D 54 24 30 48 8D 4C 24 38"), 5, true);
	if (!ue::cached::signatures::GetBoneMatrix) {
		MessageBoxA(HWND_DESKTOP, xorstr("GetBoneMatrix"), xorstr("Fortnite  "), MB_ICONERROR);
	}
	ue::cached::signatures::ProjectWorldToScreen = sdkUssage->PatternScan(ue::cached::Base, xorstr("40 53 55 56 57 41 56 48 81 EC ? ? ? ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 84 24 ? ? ? ? 33 DB 45 8A F1 49 8B F8 48 8B EA 48 8B F1 48 85 C9 0F 84"), 5, false);
	if (!ue::cached::signatures::ProjectWorldToScreen) {
		MessageBoxA(HWND_DESKTOP, xorstr("ProjectWorldToScreen"), xorstr("Fortnite  "), MB_ICONERROR);
	}
	ue::cached::signatures::FreeFn = sdkUssage->PatternScan(ue::cached::Base, xorstr("48 85 C9 0F 84 86 ? ? ? 53"), 0, false);
	if (!ue::cached::signatures::FreeFn) {
		MessageBoxA(HWND_DESKTOP, xorstr("FreeFn"), xorstr("Fortnite  "), MB_ICONERROR);
	}
	ue::cached::signatures::GetNameByIndex = sdkUssage->PatternScan(ue::cached::Base, xorstr("40 53 48 83 EC 20 33 C0 48 8B DA 48 89 02 48 89 42 08 E8 09 ? ? ? 48 8B C3 48 83 C4 20 5B C3 48 89 5C 24 08 48 89 6C"), 0, false);
	if (!ue::cached::signatures::GetNameByIndex) {
		MessageBoxA(HWND_DESKTOP, xorstr("GetNameByIndex"), xorstr("Fortnite  "), MB_ICONERROR);
	}
	ue::cached::signatures::LineSightOfTo = sdkUssage->PatternScan(ue::cached::Base, xorstr("48 8B C4 48 89 58 20 55 56 57 41 54 41 56 48 8D 68 B8 48 81 EC ?? ?? ?? ??"), 0, false);;
	if (!ue::cached::signatures::LineSightOfTo) {
		MessageBoxA(HWND_DESKTOP, xorstr("LineSightOfTo"), xorstr("Fortnite  "), MB_ICONERROR);
	}
	...
	if (!ue::classes::objects) {
		MessageBoxA(HWND_DESKTOP, xorstr("GObjects"), xorstr("Fortnite  "), MB_ICONERROR);
	}
	...
	ue::cached::objects::render::Font = (ue::classes::UObject*)find::FindObject(xorstr(L"/Game/UI/Foundation/Fonts/BurbankSmall.BurbankSmall")); if (!ue::cached::objects::render::Font) return;
